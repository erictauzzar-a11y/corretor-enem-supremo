...
O preço está centralizado em `server/products.ts` como `3700` centavos em BRL, com recorrência anual. O checkout é criado no backend em `server/billing.ts`, e o webhook assinado está registrado em `/api/stripe/webhook`. O sistema processa confirmação, assinatura ativa, cancelamento, falha de pagamento e checkout expirado.
