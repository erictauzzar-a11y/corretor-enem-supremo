...
import { BookOpen, Check, ChevronRight, Download, FileImage, FileText, GraduationCap, Lightbulb, Loader2, RotateCcw, Sparkles, Target, Upload, X } from "lucide-react";
...
const criteria = [
  ...
  { number: "05", title: "Proposta de intervenção", text: "Avalia a solução apresentada para o problema, que deve ser concreta, detalhada e compatível com os argumentos desenvolvidos. Uma proposta completa explicita agente, ação, meio ou modo de execução, finalidade ou efeito e detalhamento. Ela também deve respeitar os direitos humanos e não pode propor violência, discriminação ou supressão de direitos.", focus: "O que diferencia as notas altas: uma intervenção viável e específica, que responde ao problema e deixa claro quem fará o quê, como e com qual objetivo.", tip: "Como melhorar: use os argumentos do desenvolvimento para construir a solução e confira, no final, se os cinco elementos estão identificáveis e se há detalhamento concreto.", avoid: "Evite soluções vagas como ‘o governo deve conscientizar’, propostas impossíveis de executar e ações que não indicam responsável, meio ou finalidade.", example: "Exemplo completo: o Ministério da Educação deve ampliar a internet nas escolas públicas, por meio de repasses monitorados e formação técnica, para reduzir a desigualdade de acesso, com metas divulgadas por município." },
];
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fc] text-[#17233d]">
      ...
      <main className="mx-auto max-w-7xl px-5 pb-20 pt-10 lg:px-8 lg:pt-16">
        ...
        {result && <section className="result-section mt-16" id="resultado"><div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div className="eyebrow"><span className="eyebrow-dot" /> RESULTADO DA ANÁLISE</div><h2 className="mt-3 text-3xl font-extrabold tracking-tight">Seu diagnóstico completo</h2></div><div className="flex flex-wrap gap-2"><Button onClick={downloadPdf} className="w-fit rounded-xl bg-[#3155d8] text-white hover:bg-[#2546c3]"><Download className="mr-2 h-4 w-4" />Baixar PDF</Button><Button onClick={reset} variant="outline" className="w-fit rounded-xl border-[#dfe4ef] bg-white"><RotateCcw className="mr-2 h-4 w-4" />Nova correção</Button></div></div>
