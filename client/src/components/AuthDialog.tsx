...
export default function AuthDialog({ user, onUserChange }: AuthDialogProps) {
  ...
  const runAuth = async (action: () => Promise<{ error: { message: string } | null }>) => {
    ...
    try {
      ...
      setError(friendlyAuthError(authError instanceof Error ? authError.message : "Não foi possível concluir a autenticação."));
    } finally {
      setBusy(false);
    }
  };
  ...
  const submitEmail = async (event: React.FormEvent) => {
    ...
    await runAuth(async () => {
      if (mode === "signup") {
        ...
        if (!result.error && !result.data.session) setMessage("Cadastro criado. Confira seu e-mail para confirmar a conta.");
        return result;
      }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm" className="gap-2 rounded-full border-[#dfe4f1] bg-white text-[#3155d8]">
          <UserRound className="h-3.5 w-3.5" /> Entrar
        </Button>
      </DialogTrigger>
      <DialogContent className="rounded-3xl border-0 p-6 sm:max-w-[430px]">
        ...
        {!isSupabaseAuthConfigured && <Alert variant="destructive"><AlertDescription>A autenticação ainda não foi configurada neste ambiente.</AlertDescription></Alert>}
        ...
        <div className="space-y-4 pt-2">
          ...
          <div className="text-center text-sm text-[#66708a]">{mode === "login" ? "Ainda não tem conta?" : "Já possui cadastro?"}{" "}<button type="button" onClick={() => { setMode(mode === "login" ? "signup" : "login"); setError(""); setMessage(""); }} className="font-bold text-[#3155d8] hover:underline">{mode === "login" ? "Criar agora" : "Entrar"}</button></div>
        </div>
      </DialogContent>
    </Dialog>
  );
